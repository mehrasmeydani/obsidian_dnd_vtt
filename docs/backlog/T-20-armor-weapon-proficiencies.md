...
## Acceptance criteria
- [ ] Content schema: `proficiencies: { armor: [...], weapons: [...],
      tools: [...] }` on classes (races/backgrounds can adopt the same
      shape later — dwarf weapon training, background tools with T-08).
- [ ] Data for all 13 classes from the SRD.
- [ ] Assembly copies them onto the character (new Character fields —
      additive schema change).
- [ ] Class options step shows them as granted chips (per the user:
      grants are displayed there, they are **not** a pick — 5e RAW).
- [ ] Sheet preview lists them.
