...
## Acceptance criteria
- [x] Content store loads every `*.json` bundle from
...
- [x] Bundles merge over the built-in SRD by `id` (later sources override);
      merged content feeds the wizard everywhere it reads RACES/CLASSES/etc.
- [x] Bundle metadata (`name`, source, fetchedAt) is shown in settings, with
      per-bundle enable/disable.
- [x] No network access involved; unit tests for merge/override behavior.
