...
## Acceptance criteria
- [x] Content schema: class and subclass features carry a `level`
      (today's `traits` are level-1-only and stay as such, or migrate to
      a `features: [{level, name, description}]` shape — pick one, don't
      keep both).
- [x] Scaling features (Brutal Critical 1→2→3 dice, Rage uses/damage
...
- [x] Assembly copies every class + subclass feature with
      `level <= starting level` onto `Character.features`.
- [x] Mechanical effects that change derived numbers are applied at
...
- [x] The Class options step lists the granted (non-choice) features for
      ...
      way.
- [x] Data complete for **Barbarian 2014 + Path of the Berserker**:
...
- [x] Matrix/unit tests: level-20 barbarian carries the full feature set;
